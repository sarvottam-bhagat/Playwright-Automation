...
def get_llm(provider: str):
    if provider == 'anthropic':
        ...
        if not api_key:
            st.error("Error: GOOGLE_API_KEY is not set. Please provide a valid API key.")
            st.stop()
        
        return ChatGoogleGenerativeAI(
            model='gemini-2.0-pro-exp-02-05', google_api_key=api_key, temperature=0.0
        )
...
provider = st.radio("Select LLM Provider:", ["openai", "anthropic", "gemini"], index=0)
